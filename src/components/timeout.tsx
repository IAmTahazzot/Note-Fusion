...
export const TimeOut = ({ onTryAgain }: { onTryAgain?: () => void }) => {
  useEffect(() => {
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-white dark:bg-black flex items-center justify-center">
      <div className="space-y-3">
        ...
        <h1 className="text-xl font-semibold">Timeout</h1>
        <p className="text-sm">
          The request took too long to complete. Please try again later.
        </p>
        <div>
          <button
            onClick={onTryAgain ? onTryAgain : TryAgain}
            className="bg-black rounded-sm px-4 py-2 text-white text-sm hover:bg-black/75 mt-4"
          >
            Try again
          </button>
        </div>
